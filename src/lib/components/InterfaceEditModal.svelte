<script lang="ts">
	...
	let { open, deviceId, onclose, onsave }: Props = $props();
	let name = $state('');

	async function handleSubmit(e: SubmitEvent) {
		e.preventDefault();
		if (!name.trim()) return;
		await fetch(`/api/devices/${deviceId}/interfaces`, {
			method: 'POST',
...
<Modal {open} title="Add Interface" onclose={onclose}>
	<form onsubmit={handleSubmit}>
		<label class="block mb-4">
			<span class="text-sm text-slate-400">Interface name</span>
			<input
				type="text"
				bind:value={name}
				...
				placeholder="e.g., SFP1, Port5, GE8"
				autofocus
			/>
		</label>
